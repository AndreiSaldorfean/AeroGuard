...
struct Handshake{
  public:
    ...
    bool receive_ack(char* verification){
    ...
    bool initialize_handshake(char* device){
      send_ack(device);
      if(receive_ack(device)){
        return true;
      }
      return false;
    }
    Handshake(char* device){
      initialize_handshake(device);
    }
};
...
void loop() {
  char device[] ="Device_1"; 
  Serial.println("CEDAFVA");
  Handshake hand(device);
